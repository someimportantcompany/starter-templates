...
import vite from '../../src/frontend/dev-server';

// @ts-ignore
import { configureExpress, configureMiddleware, createServerReqContext } from '../server/common';
...
app.get('*', async (req, res, next) => {
  try {
    const { pageHtml, server: serverRes } = await (await vite).render(createServerReqContext(req));
